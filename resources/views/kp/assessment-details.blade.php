...
<div class="mb-4">
    <button onclick="history.back()" 
            class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-emerald-600 transition">
        ...
        <div class="relative z-10">
            <div class="flex justify-between items-start">
                ...
                <a href="{{ route('subjcoordinator.assessment.download-zip', $assessment->id) }}" 
                   class="bg-emerald-600 text-white px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 shadow-sm transition flex items-center gap-2">
                    <i data-lucide="download" class="w-3 h-3"></i> Download Archive
                </a>
            </div>
            ...
            <div class="flex items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-100">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm font-medium text-slate-600 truncate max-w-[200px]">
                        {{ $loop->first ? ($assessment->question_filename ?? 'No file') : ($assessment->schema_filename ?? 'No file') }}
                    </span>
                </div>
                @if($assessment->$field)
